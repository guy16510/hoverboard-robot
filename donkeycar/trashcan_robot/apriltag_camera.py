...
class AprilTagCamera:
    ...
    def __init__(
        ...
        *,
        ...
        self._device = str(self._config.get("device", "/dev/video0"))
    ...
    def _open_capture(self) -> None:
        cv2 = self._load_cv2()
        source: str | int = (
            int(self._device) if self._device.isdecimal() else self._device
        )
        ...
        if not capture or not capture.isOpened():
            if capture:
                capture.release()
            raise RuntimeError(f"unable to open backup camera {self._device}")
    ...
    def _process_frame(self, frame: Any) -> tuple[Any, list[dict[str, Any]]]:
        ...
        if ids is not None:
            for marker_corners, marker_id in zip(corners, ids.flatten()):
                ...
                label = (
                    f"tag {detection['id']} "
                    f"{detection['bearing_degrees']:+.1f}deg"
                )
        ...
        while not self._stopped.is_set():
            ...
            try:
                ...
                if not ok or frame is None:
                    raise RuntimeError(
                        f"backup camera {self._device} stopped returning frames"
                    )
