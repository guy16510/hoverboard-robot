...
inline int16_t readI16(const uint8_t *data) {
...
class FrameParser {
 public:
  ...
  void service(HardwareSerial &serial, uint32_t nowMs, Handler &&handler) {
    while (serial.available() > 0) {
      feed(static_cast<uint8_t>(serial.read()), nowMs, handler);
    }
  }
 ...
 private:
  template <typename Handler>
  void feed(uint8_t byte, uint32_t nowMs, Handler &handler) {
    if (length_ == 0) {
      if (byte == kMarker0) {
    ...
    if (length_ != expectedLength || buffer_[2] != kVersion) {
      ++rejectedFrames_;
      reset(byte);
      return;
    }
    ...
    const FrameView frame{
        buffer_[3], readU16(buffer_ + 5), buffer_ + 9, payloadLength};
    handler(frame, nowMs);
    reset();
  }
