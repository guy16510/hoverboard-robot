...
bool gs_encode_esp_command(uint8_t out[GS_ESP_COMMAND_SIZE],
  ...
  write_u16(&out[3], command->enable_epoch);
  write_u16(&out[5], command->master_clear_fault_epoch);
  write_u16(&out[7], command->slave_clear_fault_epoch);
  write_u16(&out[9], (uint16_t)command->speed);
  write_u16(&out[11], (uint16_t)command->steer);
  out[13] = command->master_flags;
  out[14] = command->slave_flags;
...
bool gs_decode_esp_command(gs_esp_command *out,
  ...
  decoded.enable_epoch = read_u16(&frame[3]);
  decoded.master_clear_fault_epoch = read_u16(&frame[5]);
  decoded.slave_clear_fault_epoch = read_u16(&frame[7]);
  decoded.speed = (int16_t)read_u16(&frame[9]);
  decoded.steer = (int16_t)read_u16(&frame[11]);
  decoded.master_flags = frame[13];
  decoded.slave_flags = frame[14];
...
bool gs_encode_slave_command(uint8_t out[GS_SLAVE_COMMAND_SIZE],
  ...
  write_u16(&out[1], command->sequence);
  write_u16(&out[3], command->enable_epoch);
  write_u16(&out[5], command->clear_fault_epoch);
  write_u16(&out[7], (uint16_t)command->electrical_command);
  out[9] = command->flags;
...
bool gs_decode_slave_command(gs_slave_command *out,
  ...
  decoded.sequence = read_u16(&frame[1]);
  decoded.enable_epoch = read_u16(&frame[3]);
  decoded.clear_fault_epoch = read_u16(&frame[5]);
  decoded.electrical_command = (int16_t)read_u16(&frame[7]);
  decoded.flags = frame[9];
...
bool gs_encode_slave_feedback(uint8_t out[GS_SLAVE_FEEDBACK_SIZE],
  ...
  write_u16(&out[4], feedback->enable_epoch);
  write_u16(&out[6], feedback->fault_epoch);
  out[8] = feedback->clear_result;
  out[9] = 0u;
  write_u16(&out[10], (uint16_t)feedback->applied_electrical);
  write_u32(&out[12], (uint32_t)feedback->odometer);
  write_u32(&out[16], feedback->faults);
  write_u32(&out[20], feedback->first_fault);
  write_u16(&out[24], feedback->command_age_ms);
...
bool gs_decode_slave_feedback(gs_slave_feedback *out,
  ...
  out->enable_epoch = read_u16(&frame[4]);
  out->fault_epoch = read_u16(&frame[6]);
  out->clear_result = frame[8];
  out->applied_electrical = (int16_t)read_u16(&frame[10]);
  out->odometer = (int32_t)read_u32(&frame[12]);
  out->faults = read_u32(&frame[16]);
  out->first_fault = read_u32(&frame[20]);
  out->command_age_ms = read_u16(&frame[24]);
  return true;
}
...
bool gs_encode_master_feedback(uint8_t out[GS_MASTER_FEEDBACK_SIZE],
  ...
  write_u16(&out[12], feedback->master_enable_epoch);
  write_u16(&out[14], feedback->slave_enable_epoch);
  write_u16(&out[16], feedback->master_fault_epoch);
  write_u16(&out[18], feedback->slave_fault_epoch);
  out[20] = feedback->master_clear_result;
  out[21] = feedback->slave_clear_result;
  write_u16(&out[22], (uint16_t)feedback->left_applied);
  write_u16(&out[24], (uint16_t)feedback->right_applied);
  write_u32(&out[26], (uint32_t)feedback->left_odometer);
  write_u32(&out[30], (uint32_t)feedback->right_odometer);
  write_u32(&out[34], feedback->master_faults);
  write_u32(&out[38], feedback->slave_faults);
  write_u32(&out[42], feedback->master_first_fault);
  write_u32(&out[46], feedback->slave_first_fault);
  write_u16(&out[50], feedback->master_command_age_ms);
  write_u16(&out[52], feedback->slave_feedback_age_ms);
  write_u16(&out[54], feedback->slave_command_age_ms);
...
bool gs_decode_master_feedback(gs_master_feedback *out,
  ...
  out->master_enable_epoch = read_u16(&frame[12]);
  out->slave_enable_epoch = read_u16(&frame[14]);
  out->master_fault_epoch = read_u16(&frame[16]);
  out->slave_fault_epoch = read_u16(&frame[18]);
  out->master_clear_result = frame[20];
  out->slave_clear_result = frame[21];
  out->left_applied = (int16_t)read_u16(&frame[22]);
  out->right_applied = (int16_t)read_u16(&frame[24]);
  out->left_odometer = (int32_t)read_u32(&frame[26]);
  out->right_odometer = (int32_t)read_u32(&frame[30]);
  out->master_faults = read_u32(&frame[34]);
  out->slave_faults = read_u32(&frame[38]);
  out->master_first_fault = read_u32(&frame[42]);
  out->slave_first_fault = read_u32(&frame[46]);
  out->master_command_age_ms = read_u16(&frame[50]);
  out->slave_feedback_age_ms = read_u16(&frame[52]);
  out->slave_command_age_ms = read_u16(&frame[54]);
  return true;
}

_Static_assert(GS_ESP_COMMAND_SIZE == 17, "ESP32 command wire size changed");
_Static_assert(GS_SLAVE_COMMAND_SIZE == 12, "slave command wire size changed");
_Static_assert(GS_SLAVE_FEEDBACK_SIZE == 28,
               "slave feedback wire size changed");
_Static_assert(GS_MASTER_FEEDBACK_SIZE == 58,
