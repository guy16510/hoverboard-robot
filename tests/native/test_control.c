...
static void test_motor_ramps_limits_stops_and_reverses(void) {
  ...
  GS_EXPECT_EQ(GS_PWM_OFFSET_START, out.demand.compare_offset);
  GS_EXPECT_TRUE(fake.last_offset <= GS_PWM_OFFSET_MAX);
  ...
  out = gs_motor_step(&motor, &(gs_motor_input){3, false, 0, true, -1000, 699});
  GS_EXPECT_EQ(0, out.demand.logical_command);
  GS_EXPECT_FALSE(out.demand.bridge_enabled);
  out = gs_motor_step(&motor, &(gs_motor_input){3, false, 0, true, -1000, 700});
